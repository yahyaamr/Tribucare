...
export default async function AdminDashboard() {
  ...
  const stats = [
    { label: t.dashboard.publishedBlogs, value: published, icon: Send },
    { label: t.dashboard.publishedNews, value: newsPublished, icon: Newspaper },
    { label: t.dashboard.drafts, value: drafts, icon: FileText },
    { label: t.dashboard.images, value: media.length, icon: Images },
  ];
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
      ...
      <p className="mt-1 text-sm text-ink-soft">{t.dashboard.intro}</p>
      ...
      <div className="card-surface mt-6 overflow-hidden">
        ...
        {recent.length === 0 ? (
          <div className="px-5 py-10 text-center">
            <p className="text-sm text-ink-soft">{t.dashboard.noBlogs}</p>
            <Link
              href={`${base}/posts/new`}
                ...
                >
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 block text-xs text-ink-faint">
                      {post.categories.join(t.common.listSeparator) || t.common.uncategorised} ·{" "}
      ...
      <div className="card-surface mt-6 overflow-hidden">
        ...
        {recentNews.length === 0 ? (
          <div className="px-5 py-10 text-center">
            <p className="text-sm text-ink-soft">{t.dashboard.noNews}</p>
            <Link
              href={`${base}/news/new`}
              className="mt-4 inline-flex items-center gap-2 rounded-xl bg-brand-700 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-brand-800"
            >
              {t.dashboard.addFirstNews}
            </Link>
          </div>
        ) : (
          <ul className="divide-y divide-brand-50">
