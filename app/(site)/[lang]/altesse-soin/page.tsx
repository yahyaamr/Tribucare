...
export default async function AltesseSoinPage() {
  const { altesse, altesseCollections, ui } =
    await content();
  const locale = await currentLocale();
  ...
  return (
    <>
        ...
        <Shell className="relative z-10 pt-30 pb-24 md:pt-36 md:pb-32 lg:pt-44">
          ...
          <div className="mt-10 -me-5 sm:-me-8 lg:hidden">
            <Image
              ...
              priority
              sizes={HERO_IMAGE_SIZES}
              className="ms-auto h-auto w-[88%] max-w-none [-webkit-mask-image:linear-gradient(to_top,transparent_0,#000_24%)] [mask-image:linear-gradient(to_top,transparent_0,#000_24%)]"
            />
          </div>
          ...
          <Reveal delay={340}>
            <div className="mt-16 border-t border-white/10 pt-8">
              <p className="eyebrow text-brand-300">{ui.sections.ownFlagshipBrand}</p>
      ...
      <section id="catalogue" className="ground-light relative py-24 md:py-32">
        <Shell>
          ...
          <ul className="mt-16 grid gap-8 sm:grid-cols-2 md:mt-24 lg:grid-cols-3">
            {altesseCollections.map((collection, i) => (
              <Reveal
                as="li"
                key={collection.slug}
      ...
      {/* Closes the page on the deep ground, so the run reads
          deep / light / deep / light / deep the whole way down. Shares its
          <ChannelCard> with the MLAY page's Distribution section, on the dark
          tone; the chip row and the CTA are the Partner section's, verbatim. */}
      <section
        id="distribution"
      ...
      >
        ...
        <Shell className="relative">
          <div className="grid gap-10 lg:grid-cols-12 lg:items-end">
            <Reveal className="lg:col-span-7">
              <Eyebrow tone="light">{altesse.distribution.eyebrow}</Eyebrow>
              <h2 className="mt-6 font-display text-[clamp(2.125rem,4.6vw,3.5rem)] font-semibold leading-[1.03] tracking-[-0.025em] text-balance text-white">
                {altesse.distribution.headlineLead}{" "}
                <span className="text-brand-300">
                  {altesse.distribution.headlineAccent}
                </span>
              </h2>
            </Reveal>
            <Reveal className="lg:col-span-5" delay={100} from="right">
              <p className="text-[1.0625rem] leading-relaxed text-brand-100/80">
                {altesse.distribution.intro}
              </p>
            </Reveal>
          </div>

          <ul className="mt-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {altesse.distribution.channels.map((channel, i) => (
              <Reveal
                as="li"
                key={channel.title}
                delay={i * 80}
          ...
          <div className="mt-16 grid gap-8 border-t border-white/10 pt-10 lg:grid-cols-12 lg:items-center">
            ...
            <Reveal className="lg:col-span-5 lg:justify-self-end" delay={100} from="right">
              ...
              >
                {altesse.distribution.partner.cta.label}
                <ArrowRight
                  className="size-4 transition-transform duration-300 group-hover:translate-x-1 rtl:group-hover:-translate-x-1"
                  aria-hidden="true"
                />
