...
import { getPublicCategories, type Category } from "@/lib/cms/categories";

/**
 * The blog index, as a component.
 *
 * Two routes render it: `/blogs` with nothing active, and `/blogs/<slug>` when
 * the slug is a category's rather than an article's. A category page *is* the
 * index filtered to it — same header, same search, same grid — so the index
 * lives here once and both routes call it, rather than the category route
 * carrying a second copy of the page that would drift from the first.
 */
export async function BlogListing({ active }: { active?: Category }) {
